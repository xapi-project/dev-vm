...
# Install opam
    ...
    # Install opam 2:
    sudo curl -sL https://github.com/ocaml/opam/releases/download/2.0.2/opam-2.0.2-x86_64-linux --output /usr/local/bin/opam
    sudo chmod +x /usr/local/bin/opam
    opam init --disable-sandboxing --auto-setup --compiler=4.07.1 --yes
    eval `opam env`
